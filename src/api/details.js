...
async function fetchWeatherData(lat, lng) {
  try {
    const response = await fetch(
      `https://api.weatheri.com/v1/forecast.json?key=${apiKeyWeatherAPI}&q=${lat},${lng}&days=3`
    );
    if (!response.ok) {
    ...
    document.getElementById(
      "riskData"
    ).innerHTML = `<p>Error fetching risk data. Please try again.</p>`;
  }
}

function displayWeatherData(data) {
  const forecasts = data.forecast.forecastday;
  const weatherHTML = forecasts
    .map((day) => {
    ...
    .map((risk) => {
      return `
            <div class="risk-day">
                ...
                <p>Heavy Rain Risk: ${
                  risk.heavyRainRisk === 3 ? "High" : "Low"
                }</p>
                <p>Tsunami Risk: ${
                  risk.tsunamiRisk === 2 ? "Possible" : "None"
                }</p>
                <p>Landslide Risk: ${
                  risk.landslideRisk === 3
                    ? "High"
                    : risk.landslideRisk === 2
                    ? "Moderate"
                    : "Low"
                }</p>
            </div>
...
async function fetchEarthquakeData(lat, lng) {
  ...
  try {
    const response = await fetch(
      `https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&latitude=${lat}&longitude=${lng}&maxradiuskm=${maxRadiusKm}`
    );
    if (!response.ok) {
    ...
    document.getElementById(
      "earthquakeData"
    ).innerHTML = `<p>Error fetching earthquake data. Please try again.</p>`;
  }
}
...
function displayEarthquakeData(data) {
  if (data.features.length === 0) {
    document.getElementById(
      "earthquakeData"
    ).innerHTML = `<p>No recent earthquakes found within 100 km.</p>`;
    return;
  }
  ...
  document.getElementById(
    "earthquakeData"
  ).innerHTML = `<h3>Recent Earthquakes (within 100 km)</h3>${earthquakes}`;
}
...
async function fetchElevationData(lat, lng) {
  try {
    ...
    document.getElementById(
      "elevationData"
    ).innerHTML = `<p>Error fetching elevation data. Please try again.</p>`;
  }
}
...
function displayElevationData(data) {
  ...
  document.getElementById(
    "elevationData"
  ).innerHTML = `<p>Elevation: ${elevation} meters</p>`;
  document
    .getElementById("elevationData")
    .setAttribute("data-elevation", elevation);
}
